...
import pickle
import pandas as pd
from contextual_bandit import Context
...
class MiniMonster(object):
    ...
    def fit(self, T, batch):
        ...
        while t < T+1:
            ...
            if t == 1:
                ...
                best_pi, log_Reg = Argmin.argmin(self.eps, self.nu, self.fairness, self.dataset1)
        ...
        mean_pred_dict = {0: self.statistics.mean_pred_0_list, 1: self.statistics.mean_pred_1_list, 'overall': self.statistics.mean_pred_overall_list}
        dictionary = {'auc': acc_dict,
                      'mean_pred': mean_pred_dict,
                      'DP':self.statistics.DP_list,
                      'TPR':self.statistics.TPR_list,
                      'FPR':self.statistics.FPR_list,
                      'EO':self.statistics.EO_list}
        # save_path = Path(base_save_path).mkdir(parents=True, exist_ok=True)
        # Evaluation.save_dictionary(dictionary, save_path)
    ...
    def sample(self, x, Q, best_pi, m):
        ...
        # in paper:
        p = (1-2*self._get_mu(m))*p + self._get_mu(m)
    ...
    def _solve_op(self, H, t, best_pi, Q = None):
        ...
        while updated and iterations < 20:
            ...
            for i in range(t):
                ...
                q = (1.0 - self._get_mu(t)) * q + (self._get_mu(t))/2
                # print('q', q)
